...
class StaffingPlan(Document):
	...
	def validate_overlap(self, staffing_plan_detail):
		# THIS WAS CORE CODE COMMENTED WHEN REMOVING MONKEYPATCH
		...
		# CUSTOM CODE FROM MONKEYPATCH REMOVAL
		# Validate if any submitted Staffing Plan exist for any Designations in this plan
		# and spd.vacancies>0 ?
		# Customization #
		# we will validate the uniqueness including the department itself
		# Customization #
		overlap = frappe.db.sql("""select spd.parent
			from `tabStaffing Plan Detail` spd join `tabStaffing Plan` sp on spd.parent=sp.name
			where spd.designation=%s and sp.docstatus=1
			and sp.to_date >= %s and sp.from_date <= %s and sp.company = %s and sp.department = %s and sp.branch = %s and sp.sub_branch = %s
		""", (staffing_plan_detail.designation, self.from_date, self.to_date, self.company, self.department, self.branch, self.sub_branch))
		if overlap and overlap [0][0]:
			frappe.throw(_("Staffing Plan '{0}' already exist for designation '{1}' in '{2}' sub branch of '{3}' branch and '{4}' department"
				.format(overlap[0][0], staffing_plan_detail.designation, self.sub_branch, self.branch, self.department)))
	...
	def validate_with_parent_plan(self, staffing_plan_detail):
		...
		# this core code was removed in monkeypatch, so commented out here when removing monkeypatch from nerp
		# original removal in prod branch commit # 82f6b9a848aeaa2fc382f9af45ed743fc26a1968
		return
...
# Core code is commented out.
# monkeypatch code moved here from nerp, while monkeypatch removal
@frappe.whitelist()
def get_active_staffing_plan_details(company, designation, from_date=getdate(nowdate()), to_date=getdate(nowdate()), department=False, branch=False, sub_branch=False):
	...
	if not company or not designation or not department or not branch or not sub_branch:
		frappe.throw(_("Please select Company, Designation, Department, Branch and Sub Branch"))

	staffing_plan = frappe.db.sql("""
		select sp.name, spd.vacancies, spd.total_estimated_cost
		from `tabStaffing Plan Detail` spd join `tabStaffing Plan` sp on spd.parent=sp.name
		where company=%s and spd.designation=%s and sp.docstatus=1
		and to_date >= %s and from_date <= %s and department=%s and branch=%s and sub_branch=%s """, (company, designation, from_date, to_date, department, branch, sub_branch), as_dict = 1)
	if not staffing_plan:
		parent_company = frappe.get_cached_value('Company',  company,  "parent_company")
		if parent_company:
			staffing_plan = get_active_staffing_plan_details(parent_company,
				designation, from_date, to_date, department, branch, sub_branch)