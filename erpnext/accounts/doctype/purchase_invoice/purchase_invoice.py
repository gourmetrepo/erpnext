...
class PurchaseInvoice(BuyingController):
            ...
            pass
             ...
             if len(self.customer_loan_deduction) > 0:
                ...
                if not self.customer_loan_deduction[0].total_payable:
                    ...
                    self.customer_loan_deduction[0].net_recieveable = flt(total_receivable) - flt(self.customer_loan_deduction[0].allocated_amount)
                    self.customer_loan_deduction[0].allocation_percentage = flt((flt(self.customer_loan_deduction[0].allocated_amount) / flt(total_receivable)) * 100)

        if not self.on_hold:
            self.release_date = ""

    def invoice_is_blocked(self):
        return self.on_hold and (
            not self.release_date or self.release_date > getdate(nowdate())
        )
    ...
    def validate(self):
        ...
        self.validate_multiple_billing(
            "Purchase Receipt", "pr_detail", "amount", "items"
        )
        self.create_remarks()
        self.set_status()
    ...
    def validate_credit_to_acc(self):
        ...
        if self.supplier and account.account_type != "Payable":
            frappe.throw(
                _(
                    "Please ensure {} account is a Payable account. \
    ...
    def check_on_hold_or_closed_status(self):
        ...
        for d in self.get("items"):
            if (
                d.purchase_order
                and not d.purchase_order in check_list
    ...
    def validate_item_code(self):
        for d in self.get("items"):
            if not d.item_code:
                frappe.msgprint(
                    _("Item Code required at Row No {0}").format(d.idx),
    ...
    def set_expense_account(self, for_validate=False):
        ...
        if len(asset_items) > 0:
            asset_received_but_not_billed = self.get_company_default(
            ...
            if (
                ...
                and (
                    ...
                    or not frappe.db.get_value(
                        "Purchase Order Item", item.po_detail, "delivered_by_supplier"
                    )
                )
            ):
        ...
        if frappe.db.get_value("Buying Settings", None, "po_required") == "Yes":
            ...
            for d in self.get("items"):
                if not d.purchase_order:
                    throw(
                        _(
                            """Purchase Order Required for item {0}
						...
						{1} as {2} in {3}"""
                            ...
                            frappe.bold("No"),
                            get_link_to_form(
                                "Buying Settings", "Buying Settings", "Buying Settings"
                            ),
                        )
                    )
        ...
        if frappe.db.get_value("Buying Settings", None, "pr_required") == "Yes":
            ...
            for d in self.get("items"):
                if not d.purchase_receipt and d.item_code in stock_items:
                    throw(
                        _(
                            """Purchase Receipt Required for item {0}
    ...
    def check_prev_docstatus(self):
        for d in self.get("items"):
            if d.purchase_order:
                ...
                if not submitted:
                    frappe.throw(
                        _("Purchase Order {0} is not submitted").format(
                            d.purchase_order
                        )
                    )
            if d.purchase_receipt:
                ...
                if not submitted:
                    frappe.throw(
                        _("Purchase Receipt {0} is not submitted").format(
                            d.purchase_receipt
                        )
                    )
    ...
    def make_gl_entries(
        self, gl_entries=None, repost_future_gle=True, from_repost=False
    ):
        ...
        if gl_entries:
            update_outstanding = (
                "No" if (cint(self.is_paid) or self.write_off_account) else "Yes"
            )
            ...
            if (
                (repost_future_gle or self.flags.repost_future_gle)
    ...
    def make_supplier_gl_entry(self, gl_entries):
        ...
        if grand_total:
            ...
            grand_total_in_company_currency = flt(
                grand_total * self.conversion_rate, self.precision("grand_total")
            )
            gl_entries.append(
                self.get_gl_dict(
                    {
        ...
        if self.update_stock:
            for d in frappe.get_all(
                ...
                voucher_wise_stock_value.setdefault(
                    d.voucher_detail_no, d.stock_value_difference
                )
                ...
                if (
                    ...
                    if landed_cost_entries:
                        for account, amount in iteritems(
                            landed_cost_entries[(item.item_code, item.name)]
                        ):
                            gl_entries.append(
                                self.get_gl_dict(
                                    {
                    ...
                    item.is_fixed_asset
                    and not is_cwip_accounting_enabled(asset_category)
                ):
                    expense_account = (
                    ...
                    if not item.is_fixed_asset:
                        amount = flt(
                            item.base_net_amount, item.precision("base_net_amount")
                        )
                    else:
                        amount = flt(
    ...
    def get_asset_gl_entry(self, gl_entries):
        ...
        for item in self.get("items"):
            if item.is_fixed_asset:
                ...
                item_exp_acc_type = frappe.db.get_value(
                ...
                if not self.update_stock:
                        ...
                        self.get_gl_dict(
                            {
                                ...
                                "against": self.supplier,
                                "remarks": self.get("remarks")
                                or _("Accounting Entry for Asset"),
                                "debit": base_asset_amount,
                                "debit_in_account_currency": (
                                    base_asset_amount
                                    if arbnb_currency == self.company_currency
                                    else asset_amount
                                ),
                                "cost_center": item.cost_center,
                                "project": item.project or self.project,
                        ...
                        self.get_gl_dict(
                            {
                                ...
                                "debit_in_account_currency": (
                                    base_asset_amount
                                    if cwip_account_currency == self.company_currency
                                    else asset_amount
                                ),
                                "cost_center": self.cost_center,
                                "project": item.project or self.project,
                    ...
                    if item.item_tax_amount and not cint(
    ...
    def make_stock_adjustment_entry(
        ...
        if (
            ...
            != flt(voucher_wise_stock_value.get(item.name), net_amt_precision)
        ):
            ...
            stock_amount = flt(
                voucher_wise_stock_value.get(item.name), net_amt_precision
            )
        ...
        for tax in self.get("taxes"):
                    ...
                    self.get_gl_dict(
                        {
                            ...
                            dr_or_cr
                            + "_in_account_currency": tax.base_tax_amount_after_discount_amount
            ...
            if (
                ...
                valuation_tax[tax.name] += (
                    tax.add_deduct_tax == "Add" and 1 or -1
                ) * flt(tax.base_tax_amount_after_discount_amount)

        if (
            self.is_opening == "No"
            and self.negative_expense_to_be_booked
            and valuation_tax
        ):
        ...
        if self.bill_no:
            if cint(
                ...
                if pi:
                    ...
                    frappe.throw(
                        _(
                            "Supplier Invoice No exists in Purchase Invoice {0}".format(
                                pi
                            )
                        )
                    )
