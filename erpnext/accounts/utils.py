...
def make_inter_unit_overhead_journal_entry(sales_invoice=None):
	try:
		...
		if inter_units_overhead_companies:
			units = "IN ({})".format(
				", ".join(f"'{c}'" for c in inter_units_overhead_companies)
			)

		sales_invoices = frappe.db.sql(
			f"""SELECT si.name AS name, si.company AS company, soi.item_category AS item_category , si.customer AS customer, si.posting_date AS posting_date, si.customer_name AS customer_name, si.total AS total, sii.delivery_note AS delivery_note FROM `tabSales Invoice` AS si
				INNER JOIN `tabSales Invoice Item` AS sii ON sii.parent=si.name
				LEFT Join `tabSales Order Item` AS soi ON soi.parent=sii.sales_order
				LEFT Join `tabSales Order` AS so ON so.name=soi.parent
				
				WHERE si.name="{sales_invoice}"
				AND si.customer_name {units}
				AND si.company {units}
				AND si.docstatus= 1
		...
		if not sales_invoices:
			frappe.log_error(title = "Inter Unit Overhead Sales Jv", message = f"Sales Invoices not found for {sales_invoices[0].get('posting_date')}.")
			return

		existing_jv = frappe.db.sql(
			f"""SELECT user_remark FROM `tabJournal Entry` WHERE posting_date="{sales_invoices[0].get('posting_date')}" AND title LIKE ('%Inter Unit Overhead Sales%') AND docstatus=1;""",
			as_dict=True
		)

		if existing_jv:
			for si in existing_jv:
				remark = si.get("user_remark")
				user_remark = remark.split(":")[1]
				si_from_remarks = [si.strip() for si in user_remark.split(",")]
				res_si.update(si_from_remarks)

			sales_invoices = [si for si in sales_invoices if si.get("name") not in res_si]

		unique_sales_invoices = list({frozenset(si.items()): si for si in sales_invoices}.values())
		sales_invoices = unique_sales_invoices

		for si in sales_invoices:
			if not company_si.get(si.get("company")):
				company_si[si.get("company")] = [si]
			else:
				company_si[si.get("company")].append(si)

		for company, sales_invoices in company_si.items():
			try:
				jv_accounts = []
				si_names = []
				for si in sales_invoices:
					...
					if si.get("item_category") != "Finished Good":
						frappe.log_error(
							title = "Invalid item category in overhead Sales Invoice",
							message = f"Invalid item category '{si.get('item_category')}' in Sales Invoice {si['name']}. Only 'Finish Good' allowed."
						)
						continue

					debt_account = f"7.01.01.001 - Gourmet Sales - U{company_no}"
					inter_company_receivables_credit_account = (
						f"2.03.02.001 - Inter Company Receivables - U{company_no}"
					)
					inter_unit_transfer_overheads_credit_account = (
						f"9.01.22.001 - Inter Unit Transfer Overheads - U{company_no}"
					)
					cogs_direct_cost_credit_account = (
						f"9.01.01.003 - COGS - Direct Cost - U{company_no}"
					)

					dn_ledger_amount = frappe.db.sql(
						f"""SELECT debit AS amount FROM `tabGL Entry` WHERE voucher_no='{si.get("delivery_note")}' AND ACCOUNT='{cogs_direct_cost_credit_account}';""",
						as_dict=True
					)

					debit_amount = int(si.get("total", 0.0))
					if len(dn_ledger_amount) > 0:
						cogs_amount = int(dn_ledger_amount[0].get("amount", 0.0))
					inter_unit_transfer_overheads_amount = int(
						cogs_amount * inter_units_overhead.get(company)
					)
					# combined amount mean cogs amount + 7% of inter unit overhead
					inter_company_receivables_combined_amount =  cogs_amount + inter_unit_transfer_overheads_amount
					credit_sales_amount = frappe.db.sql(
						f"""SELECT debit AS amount FROM `tabGL Entry` WHERE voucher_no='{si.get("name")}' AND ACCOUNT='{inter_company_receivables_credit_account}';""",
						as_dict=True
					)
					inter_company_receivables_amount = credit_sales_amount[0].get("amount")
					...
					jv_accounts.append(
						{
							...
							"credit_in_account_currency": 0.0,
							"credit": 0.0,
							"debit_in_account_currency": inter_company_receivables_combined_amount,
							"debit": inter_company_receivables_combined_amount,
							"is_advance": "No",
							"against_account": "",
							"user_remark": "",
							"cost_center": f"Main - U{company_no}",
					...
					jv_accounts.append(
						{
							...
							"party_type": "Customer",
							"party": si.get("customer"),
							"credit_in_account_currency": inter_company_receivables_amount,
							"credit": inter_company_receivables_amount,
					...
					jv_accounts.append(
						{
							...
							"party_type": "",
							"party": "",
							"credit_in_account_currency": inter_unit_transfer_overheads_amount,
							"credit": inter_unit_transfer_overheads_amount,
							"debit_in_account_currency": 0.0,
							"debit": 0.0,
							"is_advance": "No",
							"against_account": "",
				...
				jv_payload = {
					...
					"posting_date": sales_invoices[0].get("posting_date"),
					"company": company,
					"accounts": jv_accounts,
					"doctype": "Journal Entry",
					"user_remark": f"Inter Unit Journal Entry Overhead for Sales Invoice(s): {', '.join(si_names)}",
				}
				...
				frappe.log_error(
					f"Error in inter_unit_overhead_journal_entry: {str(e)}",
					title="Inter Unit Overhead Sales JV Error"
				)
				for si in sales_invoices:
					si_name = si.get("name")
					try:
						...
						frappe.db.set_value("Sales Invoice", si_name, "docstatus", 0)
						...
						frappe.log_error(
							f"Failed to revert SI {si_name} to draft: {revert_error}",
		...
		frappe.log_error(
			f"Error in overhead_jv: {str(e)}", title="Inter Unit Overhead Sales JV Error"
		)
...
def make_inter_unit_sales_journal_entry(sales_invoice=None):
	try:
		...
		#     "sales_invoices_list": ["ACC-SINV-2025-457270", "ACC-SINV-2025-457271"]
		#     }
		# }
		...
		if inter_units_overhead_companies:
			units = ", ".join(f"'{c}'" for c in inter_units_overhead_companies)
		...
		query = f"""
					...
					LEFT Join `tabSales Order` AS so ON so.name=soi.parent
					WHERE
					si.customer_name IN ({units})
					AND si.name = "{sales_invoice}"
					AND si.docstatus = 1
					AND so.transaction_date >= '2025-05-05'
					AND si.outstanding_amount != 0
					{condition}
		...
		if not sales_invoices:
			frappe.log_error(message = f"No Sales Invoices found for Inter Unit Sales JV for {sales_invoices[0].get('posting_date')}", title="Inter Unit Sales JV")
			return

		existing_jv = frappe.db.sql(
			"""SELECT user_remark FROM `tabJournal Entry` 
			   WHERE posting_date = %s AND title LIKE %s AND docstatus = 1""",
			(sales_invoices[0].get('posting_date'), "%Inter Unit Sales JV for %"),
			as_dict=True
		)
		...
		if existing_jv:
			for existing_jvs in existing_jv: 
				remark = existing_jvs.get("user_remark", "")
				if ":" in remark:
					user_remark = remark.split(":", 1)[1]
					si_list = [si.strip() for si in user_remark.split(",") if si.strip()]
					existing_si_set.update(si_list)

		unique_sales_invoices = [si for si in sales_invoices if si.get("name") not in existing_si_set]

		final_sales_invoices = []
		seen = set()

		for si in unique_sales_invoices:
			if si.get("name") not in seen:
				seen.add(si.get("name"))
				final_sales_invoices.append(si)

		sales_invoices = final_sales_invoices
		...
		for company, invoice_list in company_si.items():
			try:
				jv_accounts = []
				si_names = []

				for si in invoice_list:
					if si.get("so_item_category") == "Finished Good":
						frappe.log_error(
						title = "Invalid item category in Inter Unit Sales JV Error",
							message =f"Invalid item category '{si.get('so_item_category')}' in Sales Invoice {si['name']}. Only 'Non Finish Good' allowed."
						)
						continue
						
					comp = company.split(" ")
					company_no = comp[1]
					gl_amount = fetch_sales_amount_from_gl_entry(
						"Sales Invoice", si.get("name")
					)
					jv_amount = int(gl_amount.get("amount", 0.0))
					dn_data = get_delivery_note_from_sales_invoice(si)
					rce_amount = fetch_receviables_from_gl_entry(
						"Sales Invoice", si.get("name")
					)
					...
					jv_accounts.append(
						{
							...
							- dn_data.get("amount", 0.0),
							"credit": jv_amount - dn_data.get("amount", 0.0),
							"reference_type": "Sales Invoice",
							"reference_name": si.get("name"),
							"debit_in_account_currency": "",
							"debit": "",
				...
				frappe.log_error(
					f"Error in overhead_jv: {str(e)}", title="Inter Unit Sales JV Error"
				)
...
@frappe.whitelist()
def make_inter_unit_overhead_purchase_journal_entry(
	...
	try:
		...
		purchase_invoices = frappe.db.sql(
			f"""SELECT pi.name AS name, pi.company AS company, pri.item_category AS item_category, pi.supplier AS supplier, pi.supplier_name AS supplier_name, pi.total AS total, pii.purchase_order AS purchase_order FROM `tabPurchase Invoice` AS pi
				...
				WHERE 
				pi.supplier_name {units}
				AND pi.name  = "{purchase_invoice}"
		...
		if not purchase_invoices:
			frappe.log_error(title = "Inter Unit Purchase Overhead", message = f"No Purchase Invoices found {purchase_invoices[0].get('posting_date')}.")
			return

		existing_jv = frappe.db.sql(
			""" SELECT user_remark FROM `tabJournal Entry` WHERE posting_date = %s AND title LIKE %s AND docstatus = 1 """,
			(purchase_invoices[0].get('posting_date'), "%Inter Unit Overhead Purchase%"),
			as_dict=True
		)
		...
		if existing_jv:
			...
			for existing_item in existing_jv:
				remark = existing_item.get("user_remark", "")
				if ":" in remark:
					user_remark = remark.split(":", 1)[1]
					pi_from_remarks = [pi.strip() for pi in user_remark.split(",") if pi.strip()]
					processed_pis.update(pi_from_remarks)

			purchase_invoices = [pi for pi in purchase_invoices if pi.get("name") not in processed_pis]

		unique_purchase_invoices = list({frozenset(si.items()): si for si in purchase_invoices}.values())
		purchase_invoices = unique_purchase_invoices


		for pi in purchase_invoices:
			if not company_pi.get(pi.get("company")):
				company_pi[pi.get("company")] = [pi]
			else:
				company_pi[pi.get("company")].append(pi)

		for company, purchase_invoices in company_pi.items():
			try:
				jv_accounts = []
				po_names = []
				for pi in purchase_invoices:
					...
					if pi.get("item_category") != "Finished Good":
						frappe.log_error(
							title = "Invalid item category in Overhead Purchase Invoice",
							message = f"Invalid item category '{pi.get('item_category')}' in Sales Invoice {pi['name']}. Only 'Finish Good' allowed."
						)
						continue
				...
				jv_payload = {
					...
					"voucher_type": "Inter Company Journal Entry",
					"naming_series": "ACC-IUJV-.YYYY.-",
					"posting_date": purchase_invoices[0].get("posting_date"),
					"company": company,
					"accounts": jv_accounts,
					"doctype": "Journal Entry",
					"user_remark": f"Inter Unit Journal Entry Overhead for Purchase Invoice(s): {','.join(po_names)}",
				}
				journal_entry_overhead = frappe.get_doc(jv_payload)
				journal_entry_overhead.save(ignore_permissions=True,  ignore_workflow=True)
				frappe.db.commit()
...
def get_purchase_invoice_ledger(pi_name, inter_unit_overhead):
	try:
		...
		return (
			int(amount[0].get("amount") * inter_unit_overhead),
			pi_name,
		)
	except Exception as e:
		frappe.log_error(
