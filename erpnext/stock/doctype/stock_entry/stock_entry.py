...
class MaxSampleAlreadyRetainedError(frappe.ValidationError):
	pass
class ExtraMaterialReceived(frappe.ValidationError):
	pass
		...
		for d in item_dict:
			...
			if self.work_order !=None:
				_cost_center = frappe.db.sql(f"""SELECT cost_center FROM `tabSection Warehouse` AS sw
												INNER JOIN `tabWork Order` AS wo ON wo.item_section =sw.`parent` AND wo.`name`={self.work_order}""",as_dict())
			...
			se_child.cost_center = _cost_center or (item_dict[d].get("cost_center") or
