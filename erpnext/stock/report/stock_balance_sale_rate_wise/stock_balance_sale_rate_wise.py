...
def execute(filters=None):
	...
	message = """Current Sale Rate Wise Stock Value & Balance for FG & Shipping Warehouses Only """
	data = []
	conversion_factors = {}
...
def get_columns(filters):
	...
	columns = [
		...
		{"label": _("Sales Rate"), "fieldname": "val_rate", "fieldtype": "Currency", "width": 90, "convertible": "rate", "options": "currency"},
...
def get_stock_ledger_entries(filters, items):
	...
	return frappe.db.sql("""
		select
			sle.item_code, warehouse, sle.posting_date, sle.actual_qty, IFNULL(ip.price_list_rate, 0.001) as valuation_rate,
			sle.company, sle.voucher_type, sle.qty_after_transaction, (IFNULL(ip.price_list_rate, 0.001)*sle.actual_qty) as stock_value_difference,
			sle.item_code as name, sle.voucher_no, sle.stock_value
		from
			`tabStock Ledger Entry` sle
		LEFT JOIN `tabItem Price` AS ip ON ip.item_code = sle.item_code AND price_list='Lahore Shops Price List 1-Jun-2021' AND ip.valid_from = (SELECT MAX(valid_from) 
                         FROM `tabItem Price` 
                         WHERE item_code = sle.item_code AND price_list = 'Lahore Shops Price List 1-Jun-2021')
